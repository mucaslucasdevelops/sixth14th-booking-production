export async function syncLodgifyData(store, options) {
  ...
  const iCalUrl = options.iCalUrl || options.icalUrl;
  if (!apiKey && !iCalUrl) {
    throw lodgifyConfigError("Lodgify API key or iCal URL is not configured.");
  }
  ...
  if (apiKey) {
    try {
      ...
      lodgifyReservations = buildBookingSummaries(lodgifyBookings, now);
      successfulSourceCount += 1;
    } catch (error) {
      warnings.push(safeLodgifyMessage(error));
    }
  ...
  if (iCalUrl) {
    try {
      const iCalBlocks = await fetchIcalBlocks({ fetchImpl, iCalUrl, now, months });
      availabilityBlocks.push(...iCalBlocks);
      successfulSourceCount += 1;
    } catch (error) {
      warnings.push(safeLodgifyMessage(error));
    }
  ...
  if (!successfulSourceCount) {
    throw lodgifyConfigError(
      `${warnings.join(" ")} Add a Lodgify iCal export URL if the Public API key cannot read reservations.`
    );
  }
  ...
  const mergedAvailabilityBlocks = mergeAvailabilityBlocks(availabilityBlocks);
...
async function fetchIcalBlocks({ fetchImpl, iCalUrl, now, months }) {
  const response = await fetchImpl(iCalUrl, { headers: { accept: "text/calendar,*/*" } });
  const text = await response.text();
  if (!response.ok) {
    throw lodgifyHttpError("Lodgify iCal sync", response.status, text);
  }
  return parseIcalBlocks(text, now, months);
}
...
function parseIcalBlocks(text, now, months) {
  const events = parseIcalEvents(text);
  const blocks = [];
  const today = startOfUtcDay(now);
  const todayIso = dateToIso(today);
  const finalDayIso = dateToIso(addMonths(today, months));

  for (const event of events) {
    const start = normalizeIcalDate(event.DTSTART);
    const exclusiveEnd = normalizeIcalDate(event.DTEND) || (start ? addDaysIso(start, 1) : "");
    if (!start || !exclusiveEnd || exclusiveEnd <= start) continue;
    if (exclusiveEnd <= todayIso || start >= finalDayIso) continue;

    const blockStart = maxIso(start, todayIso);
    const blockEnd = minIso(exclusiveEnd, finalDayIso);
    const displayEnd = addDaysIso(blockEnd, -1);
    const summary = cleanIcalText(event.SUMMARY || event.DESCRIPTION || "");
    const reason = iCalReason(summary);
    blocks.push({
      id: `lodgify-ical-${blockStart}-${displayEnd}-${slug(reason)}`,
      source: "lodgify_availability",
      start: blockStart,
      end: blockEnd,
      displayEnd,
...
function normalizeIcalDate(value) {
  if (!value) return "";
...
function iCalReason(summary) {
  return /closed period|owner|maintenance|block/i.test(summary)
    ? "Imported Lodgify closed period"
    : "Imported Lodgify booking";
}
